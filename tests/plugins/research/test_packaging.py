"""Signed-OCI packaging tests for the F11 research plugins.

Same discipline as the E7 reference packaging: each plugin packages as
a deterministic OCI archive (§10.5) — SPDX SBOM, Ed25519 detached
signature, manifest.json + plugin source in the layer. Tests prove
end-to-end verification, byte-level determinism, and tamper rejection.
"""
...
from evoruntime.plugins.research import (
    RESEARCH_PLUGIN_MODULES,
    build_research_image,
    load_research_plugin,
)
from tests.plugins.research.support import RESEARCH_MODULE_NAMES, load_plugin_module
...
class TestPackaging:
    ...
    def test_image_verifies_end_to_end(self, module_name: str) -> None:
        ...
        image = build_research_image(module, _signing_key())
    ...
    def test_image_is_deterministic(self, module_name: str) -> None:
        ...
        module = load_plugin_module(module_name)
        key = _signing_key()
        first = build_research_image(module, key)
        second = build_research_image(module, key)
    ...
    def test_layer_carries_manifest_and_source(self, module_name: str) -> None:
        ...
        assert manifest_payload["entrypoint"]["command"] == [
            "python",
            "-m",
            f"evoruntime.plugins.research.{module_name}",
        ]
